...
import { createQuestion, deleteQuestion, listQuestions, validateQuestionInput } from '../services/questionBank.service.js';
...
quizRouter.get('/questions', (_request, response) => {
  ...
  response.json({ questions, count: questions.length, categories });
});
...
quizRouter.get('/quiz', (request, response) => {
  ...
  const count = Number.isFinite(requestedCount) ? Math.min(Math.max(requestedCount, 1), 20) : 5;
  ...
  const selected = questions.slice(0, count).map(({ correctOptionIndex: _answer, explanation: _explanation, createdAt: _createdAt, ...question }) => question);
  response.json({ questions: selected, count: selected.length });
});

quizRouter.post('/quiz/submit', (request, response) => {
  const rawAnswers = (request.body as { answers?: unknown } | null)?.answers;
  if (!Array.isArray(rawAnswers) || rawAnswers.length === 0 || rawAnswers.length > 20) {
